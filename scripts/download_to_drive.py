...
being written. Interrupted downloads resume: re-run the same command.
    ...
    python scripts/download_to_drive.py --account you@   # lego, chair, ship (~595 MB)
    python scripts/download_to_drive.py --all            # all 8 scenes (~1.3 GB)
    python scripts/download_to_drive.py --check          # report what Drive already has
"""
...
def pick_drive(account=None):
    ...
    sys.exit("Several Google accounts are signed in:\n"
             ...
             + "\n\nPick one with --account, e.g.\n"
               f"    python scripts/download_to_drive.py --account {next(iter(accounts))}\n\n"
